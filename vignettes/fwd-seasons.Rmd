...
```{r, alb-data}
load("/home/laurence-kell/Desktop/flr/FLCandy/data/alb.RData")
```
...
control <- as(FLQuants(fbar=unitMeans(fbar(alb)[,-1])*c(rep(1,50),rep(0.5,dim(fbar(alb))[2]-51))), 'fwdControl')
...
ss.file ="/home/laurence-kell/Desktop/projects/mrag/climate/task1/data/iccat/bet/2018/1-steepness0.7_MRef_sigmaR0.2_LengthLambda0.1_iter1"
...
```{r, bet-load}
load("/home/laurence-kell/Desktop/flr/FLCandy/data/bet.RData")
```
...
sr['a', , 1, 1]  <- c(rec(bet)[, -1, 1, 1, drop=TRUE] )
sr['a', , 2, 2]  <- c(rec(bet)[, -1, 2, 2, drop=TRUE] )
sr['a', , 3, 3]  <- c(rec(bet)[, -1, 3, 3, drop=TRUE] )
sr['a', , 4, 4]  <- c(rec(bet)[, -1, 4, 4, drop=TRUE] )
