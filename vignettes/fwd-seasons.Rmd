...
```{r, ple4-control}
control=fwdControl(
  lapply(2017:2050, function(x) list(year=x, quant="fbar", value=refpts(ple4.eq)["msy","harvest"]/4, season=1:4)))
```
...
Recruitent is by year, unit & season, just provide the estimated recruitments

```{r, bet-hnd}
sr=FLPar(NA, dimnames=list(params='a', year=1951:2017, unit=1:4, season=1:4, iter=1))

# Set spawning seasons
sr['a', , 1, 1]  <- c(rec(bet[[1]])[, -1, 1, 1, drop=TRUE] )
sr['a', , 2, 2]  <- c(rec(bet[[1]])[, -1, 2, 2, drop=TRUE] )
sr['a', , 3, 3]  <- c(rec(bet[[1]])[, -1, 3, 3, drop=TRUE] )
sr['a', , 4, 4]  <- c(rec(bet[[1]])[, -1, 4, 4, drop=TRUE] )

rec=predictModel(model=rec~a, params=sr)

# project for obsevered F
control=as(FLQuants(fbar=unitMeans(fbar(bet[[1]])[,-1])), 'fwdControl')

bet.fwd=fwd(bet[[1]], control=control, sr=rec)
...
params(bet[[2]])["v"]/params(bet[[2]])["R0"]

## Time series of recruits and SSB by season/unit
bet[[1]]=window(bet[[1]],start=1980)
dat=merge(transmute(subset(as.data.frame(rec(bet[[1]]),drop=T),season==unit),year=year,unit=unit,rec=data),
          transmute(subset(as.data.frame(ssb(bet[[1]]),drop=T),season==unit),year=year,unit=unit,ssb=data),
      by=c("year","unit"))
...
ratio=apply(catch(bet[[1]])[,ac(2013:2017)],4,mean)/sum(apply(catch(bet[[1]])[,ac(2013:2017)],4,mean))
